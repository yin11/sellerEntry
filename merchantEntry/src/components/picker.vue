<template>
    ...
    <section class="showChose" v-show="showPickerState">
      <section class="address bg-f">
        ...
        <ul class="fz14">
          <li class="addList" v-for="(v,k) in info" @click="getProvinceId(v.code , v.id, v.name, k)" v-show="showProvince" :class="v.selected ? 'active' : ''">{{v.name}}<i class="right-arrow"></i></li>
          <li class="addList" v-for="(v,k) in showCityList" @click="getCityId(v.code , v.id, v.name, k)" v-show="showCity" :class="v.selected ? 'active' : ''">{{v.name}}<i class="right-arrow"></i></li>
          <li class="addList" v-for="(v,k) in showDistrictList" @click="getDistrictId(v.code , v.id, v.name, k)" v-show="showDistrict" :class="v.selected ? 'active' : ''">{{v.name}}</li>
        </ul>
      </section>
    </section>
</template>
