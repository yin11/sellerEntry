<template>
	<div>
		...
		<div class="info-box">
			...
			<ul class="fz14">
				...
				<li class="border-buttom code-box">
					...
					<button type="button" class="btn-code btn bg-blue" @click="sendMess" v-show="!computedTime">获取验证码</button>
		...
		<!--下一步-->
		<div>
			<button type="button" v-if="next != '1-1-1-1-1'" class="btn bg-blue btn-layer fz18">下一步</button>
...
export default {
		...
		/*获取验证码*/
	  	sendMess(){
	  		...
	  		if(comm.rule.phone(this.userName)){
	  			...
	  			axios.post( '/sapi/voip/v2/smsSecurityCode',
	                _data,
	                {
	                headers: {
	...
	mounted(){
		...
		if(comm.getStore('RETURNUSERINFO')){// app登录进去，此时审核未通过,页面从接口获取数据回填
			...
			_self.userName = comm.isNVL(_json.userName , '');
			_self.password = comm.isNVL(_json.password , '');
			_self.repassword  = comm.isNVL(_json.password  , '');
			_self.contactPhone  = comm.isNVL(_json.contactPhone , '');
			_self.next = '1-1-1-1-0' ;
